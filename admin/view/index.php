<?php if (! PAGEDOT ) exit; ?>
<?php $folders = explode('/',$_SERVER['REQUEST_URI']);
  ...
  ?>
<?php $PAGEDOTPATH = $protocol.'://'.$_SERVER['HTTP_HOST'].$folder; ?>
<!DOCTYPE html>
...
<head>
  ...
  <title>PAGEDOT</title>
  <link rel="stylesheet" href="<?php echo $pagedot_dir; ?>/css/jquery.scrollbar.css">
  <link rel="stylesheet" href="<?php echo $pagedot_dir; ?>/css/pagedot.css?<?php echo time(); ?>">
  <link rel="stylesheet" href="https://unpkg.com/@icon/themify-icons/themify-icons.css">
  <!-- CSS only -->
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-BmbxuPwQa2lc/FVzBcNJ7UAyJxM6wuqIj61tLrc4wSX0szH/Ev+nYRRuWlolflfl" crossorigin="anonymous">

  <link href="<?php echo $pagedot_dir; ?>/css/jqColor.css" media="all" rel="stylesheet" type="text/css" />
</head>
<body>
  ...
  <input type="hidden" id="pagedot-setting-active-element-pdeid" value="">
    ...
    <div class="pagedot-container top">
      <div class="pagedot-row">
       ...
       <a class="pagedot-logo" href="https://pagedot.ru" target="_blank"><span style="color:#555"><?php echo Config('name'); ?></span> <span style="color:#ccc"><?php echo $version; ?></span></a>
        ...
        <div class="pagedot-btns-min d-none d-md-inline-flex">
          ...
          <?php if (isset(Config('buttons')['parse']) && Config('buttons')['parse'] == 1) { ?><span class="ajax-panel-unit-parse"><a href="admin.php?p=<?php echo $_GET['p'] ?? 'index.html'; ?>&action=parse" class="pagedot-btn-min pagedot-control-btn-constructor" title="Копирование сайта по ссылке"><i class="ti ti-world"></i></a></span><?php } ?>
          ...
          <?php if (isset(Config('buttons')['pages']) && Config('buttons')['pages'] == 1) { ?><button type="button" class="pagedot-btn-min pagedot-control-btn-files ajax-panel-unit-pages" onclick="PagedotBtn(this, 'files')" title="Страницы сайта"><i class="ti ti-files"></i></button><?php } ?>
          <?php if (isset(Config('buttons')['setting']) && Config('buttons')['setting'] == 1) { ?><button type="button" class="pagedot-btn-min pagedot-control-btn-config ajax-panel-unit-setting" onclick="PagedotBtn(this, 'config')" title="Общие настройки"><i class="ti ti-settings"></i></button><?php } ?>
          
          <?php if (isset(Config('buttons')['history']) && Config('buttons')['history'] == 1) { ?><button type="button" class="pagedot-btn-min pagedot-control-btn-history ajax-panel-unit-history" onclick="PagedotBtn(this, 'history')" title="История"><i class="ti ti-loop"></i></button><?php } ?>
          ...
          <?php if (isset(Config('buttons')['control']) && Config('buttons')['control'] == 1) { ?><button type="button" class="pagedot-btn-min pagedot-control-btn-controlclick ajax-panel-unit-control" onclick="PagedotBtn(this, 'controlclick')" title="Отключить контроль элементами"><i class="ti ti-hand-open"></i></button><?php } ?>

          <?php if (isset(Config('buttons')['hide']) && Config('buttons')['hide'] == 1) { ?><button type="button" class="pagedot-btn-min pagedot-control-btn pagedot-control-btn-eye ajax-panel-unit-hide" disabled="disabled" onclick="PagedotBtn(this, 'eye')" title="Временно спрятать блок"><i class="ti ti-eye"></i></button><?php } ?>

          <?php if (isset(Config('buttons')['remove']) && Config('buttons')['remove'] == 1) { ?><button type="button" class="pagedot-btn-min pagedot-control-btn pagedot-control-btn-remove ajax-panel-unit-remove" disabled="disabled" onclick="PagedotBtn(this, 'remove')" title="Удалить элемент"><i class="ti ti-trash"></i></button><?php } ?>

          <?php if (isset(Config('buttons')['clone']) && Config('buttons')['clone'] == 1) { ?><button type="button" class="pagedot-btn-min pagedot-control-btn pagedot-control-btn-duplicate ajax-panel-unit-clone" disabled="disabled" onclick="PagedotBtn(this, 'duplicate')" title="Дублировать элемент"><i class="ti ti-layers"></i></button><?php } ?>

          <?php if (isset(Config('buttons')['drag']) && Config('buttons')['drag'] == 1) { ?><button type="button" class="pagedot-btn-min pagedot-control-btn pagedot-control-btn-up ajax-panel-unit-drag" disabled="disabled" onclick="PagedotBtn(this, 'up')" title="Переместить вверх по сетке"><i class="ti ti-angle-up"></i></button>
          <button type="button" class="pagedot-btn-min pagedot-control-btn pagedot-control-btn-down ajax-panel-unit-drag" disabled="disabled" onclick="PagedotBtn(this, 'down')" title="Переместить вниз по сетке"><i class="ti ti-angle-down"></i></button><?php } ?>
          
          <?php if (isset(Config('buttons')['style']) && Config('buttons')['style'] == 1) { ?><button type="button" class="pagedot-btn-min pagedot-control-btn pagedot-control-btn-style ajax-panel-unit-style" disabled="disabled" onclick="PagedotBtn(this, 'style')" title="Стили элемента"><i class="ti ti-palette"></i></button><?php } ?>
          ...
          <?php if (isset(Config('buttons')['setting_element']) && Config('buttons')['setting_element'] == 1) { ?><button type="button" class="pagedot-btn-min pagedot-control-btn pagedot-control-btn-setting ajax-panel-unit-setting_element" disabled="disabled" onclick="PagedotBtn(this, 'setting')" title="Настройки элемента"><i class="ti ti-pencil"></i></button><?php } ?>

        </div>
        <div class="h-100 ajax-panel-unit-adaptive" style="height: 100%;">
          <?php if (isset(Config('buttons')['adaptive']) && Config('buttons')['adaptive'] == 1) { ?>
          ...
          <button class="pagedot-btn-size" title="Телефон" data-type="mobile" onclick="resizeFrame(this, 'mobile')"><i class="ti ti-mobile"></i></button><?php } ?>



        </div>
        <div><span class="pagedot-save-result"></span><button class="pagedot-editor-btn" data-page="<?php echo $page_name;?>" disabled="disabled">Сохранить</button>
        <a href="<?php echo $pagedot_dir; ?>/logout.php" class="pagedot-btn-logout b-0" title="Выход тут"><i class="ti ti-power-off"></i></a>
...
<?php } ?>
    ...
    <div class="pagedot-control-panel pagedot-control-panel-history">
      ...
      <div class="pagedot-history-item">При каждом сохранении, создается копия страницы, в любой момент Вы можете ходить по сохарненным версиям. </div>
      ...
      <div class="pagedot-control-panel-title">История</div>
        <div class="pagedot-control-history-wrapper">
         <div class="scrollbar-inner scrollbar-inner-history">
          ...
          <script>
            setTimeout(function(){
              loadHistory('<?php echo $page_name; ?>');
            }, 2000);
          </script>
         </div>
        </div>
...
<div class="tab-pane fade show active" id="tab-settings" role="tabpanel" aria-labelledby="tab-settings">
    ...
    <div class="pagedot-control-menuMobile pagedot-control-panel-menuMobile">
      ...
      <div class="pagedot-control-panel-items d-flex flex-column">
          ...
          <a href="<?php echo $pagedot_dir; ?>/logout.php" class="pagedot-btn-min pagedot-btn-min-mobile" title="Выход тут"><i class="ti ti-power-off"></i></a>
      ...
      <div class="tab-pane fade show active" id="form-t-system" role="tabpanel" aria-labelledby="form-t-system">
        ...
        <div class="row pagedot-control-panel-item">
          ...
          <div class="col-12">
            ...
            <div class="pagedot-control-panel-input">
              <input type="text" name="key" class="form-control" value="<?php echo Config('key');?>">
              <small>Ключ нужен для полного доступа ко всем функциям PageDot. <br>Ключ можно получить <a href="https://pagedot.ru/cabinet" target="_blank">в личном кабинете</a></small>
            </div>
          </div>
          ...
          <div class="col-6">
            ...
            <div class="pagedot-control-panel-input">
              <input type="text" name="login" class="form-control" value="<?php echo Config('login');?>">
            </div>
          </div>
          <div class="col-6">
            <div class="pagedot-control-panel-label">Пароль</div>
            <div class="pagedot-control-panel-input">
              <input type="text" name="password" class="form-control" value="<?php echo Config('password');?>">
            </div>
          </div>
          ...
          <div class="col-12 pt-3">
            <div class="pagedot-control-panel-title ml-0 mr-0">Доступные кнопки на панеле</div>
          </div>
          <div class="col-6">
            <label class="custom-checkbox">
              <input type="checkbox" class="ajax-custom-checkbox" value="1" name="buttons[parse]" <?php if (isset(Config('buttons')['parse']) && Config('buttons')['parse'] == 1) { echo 'checked'; } ?>>
          ...
          <div class="col-6">
            <label class="custom-checkbox">
              <input type="checkbox" class="ajax-custom-checkbox" value="1" name="buttons[pages]" <?php if (isset(Config('buttons')['pages']) && Config('buttons')['pages'] == 1) { echo 'checked'; } ?>>
              <span class="checkmark"></span>
              Страницы сайтов
            </label>
          </div>
          <div class="col-6">
            <label class="custom-checkbox">
              <input type="checkbox" class="ajax-custom-checkbox" value="1" name="buttons[setting]" <?php if (isset(Config('buttons')['setting']) && Config('buttons')['setting'] == 1) { echo 'checked'; } ?>>
              <span class="checkmark"></span>
              Общие настройки
            </label>
          </div>
          <div class="col-6">
            <label class="custom-checkbox">
              <input type="checkbox" class="ajax-custom-checkbox" value="1" name="buttons[control]" <?php if (isset(Config('buttons')['control']) && Config('buttons')['control'] == 1) { echo 'checked'; } ?>>
              <span class="checkmark"></span>
              Контроль элементов
            </label>
          </div>
          <div class="col-6">
            <label class="custom-checkbox">
              <input type="checkbox" class="ajax-custom-checkbox" value="1" name="buttons[hide]" <?php if (isset(Config('buttons')['hide']) && Config('buttons')['hide'] == 1) { echo 'checked'; } ?>>
              <span class="checkmark"></span>
              Скрытие блоков
            </label>
          </div>
          <div class="col-6">
            <label class="custom-checkbox">
              <input type="checkbox" class="ajax-custom-checkbox" value="1" name="buttons[remove]" <?php if (isset(Config('buttons')['remove']) && Config('buttons')['remove'] == 1) { echo 'checked'; } ?>>
              <span class="checkmark"></span>
              Удаление элементов
            </label>
          </div>
          <div class="col-6">
            <label class="custom-checkbox">
              <input type="checkbox" class="ajax-custom-checkbox" value="1" name="buttons[clone]" <?php if (isset(Config('buttons')['clone']) && Config('buttons')['clone'] == 1) { echo 'checked'; } ?>>
              <span class="checkmark"></span>
          ...
          <div class="col-6">
            <label class="custom-checkbox">
              <input type="checkbox" class="ajax-custom-checkbox" value="1" name="buttons[drag]" <?php if (isset(Config('buttons')['drag']) && Config('buttons')['drag'] == 1) { echo 'checked'; } ?>>
              <span class="checkmark"></span>
          ...
          <div class="col-6">
            <label class="custom-checkbox">
              <input type="checkbox" class="ajax-custom-checkbox" value="1" name="buttons[style]" <?php if (isset(Config('buttons')['style']) && Config('buttons')['style'] == 1) { echo 'checked'; } ?>>
              <span class="checkmark"></span>
              Стили элементов
            </label>
          </div>
          <div class="col-6">
            <label class="custom-checkbox">
              <input type="checkbox" class="ajax-custom-checkbox" value="1" name="buttons[setting_element]" <?php if (isset(Config('buttons')['setting_element']) && Config('buttons')['setting_element'] == 1) { echo 'checked'; } ?>>
              <span class="checkmark"></span>
              Настройки элементов
            </label>
          </div>
          <div class="col-6">
            <label class="custom-checkbox">
              <input type="checkbox" class="ajax-custom-checkbox" value="1" name="buttons[adaptive]" <?php if (isset(Config('buttons')['adaptive']) && Config('buttons')['adaptive'] == 1) { echo 'checked'; } ?>>
              <span class="checkmark"></span>
              Кнопки устройств
            </label>
          </div>
          <div class="col-6">
            <label class="custom-checkbox">
              <input type="checkbox" class="ajax-custom-checkbox" value="1" name="buttons[history]" <?php if (isset(Config('buttons')['history']) && Config('buttons')['history'] == 1) { echo 'checked'; } ?>>
              <span class="checkmark"></span>
              История
            </label>
          </div>
...
.custom-checkbox .checkmark:after {
  ...
  <iframe src="admin.php?page=<?php echo $page_name;?>&preload=true" id="PAGEDOT-CONTAINER" width="100%" frameborder="0" style="height: calc(100%)!important; width: 100% border: 0; margin-top: 0;">УПС... Ваш браузер не поддерживает плавающие фреймы!</iframe>

  <form action="" id="PAGEDOTFORM">
  <input type="hidden" name="page" value="<?php echo $page_name;?>">
  <textarea name="html" id="PAGEDOTCONTAINERTEMPLATE" style="display:none;width:100%" rows="40"><?php echo file_get_contents($sitePath.'/admin.php?page='.$page_name); ?></textarea>
...
<div class="pagedot-blockparse">
  ...
  <script src="<?php echo $pagedot_dir; ?>/js/jquery-1.5.min.js"></script>
  <script>
    if(!navigator.onLine) {
      window.stop();
...
$(".pdedraggable").draggable({
  ...
  <script src="<?php echo $pagedot_dir; ?>/js/jquery.scrollbar.js"></script>



<script type="text/javascript" src="<?php echo $pagedot_dir; ?>/js/colors.js"></script>
<script type="text/javascript" src="<?php echo $pagedot_dir; ?>/js/colorPicker.data.js"></script>
<script type="text/javascript" src="<?php echo $pagedot_dir; ?>/js/colorPicker.js"></script>
<script type="text/javascript" src="<?php echo $pagedot_dir; ?>/js/jsColor.js"></script>

<script>
  var pagedot_api_url = '<?php echo Config('api_url');?>';
  var pagedot_dir = '<?php echo Config('dir');?>';

</script>

<script src="<?php echo $pagedot_dir; ?>/js/pagedot.js?<?php echo time();?>"></script>
